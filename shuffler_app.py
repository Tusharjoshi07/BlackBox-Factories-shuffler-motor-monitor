...
def analyse_one_mode(new_cycles_mode,historical_mode,baseline_info,day_label,mode):
    ...
    for seq_key,metrics in [
        ...
        for label,b_col,c_col in metrics:
            ...
            # For duration columns: exclude 0-duration cycles
            # (caused by file boundary issue where forward move
            #  was in previous CSV file — not real zero durations)
            if "duration" in b_col:
                valid_base = base_cycles[base_cycles[b_col] > 0]
                valid_curr = new_cycles_mode[new_cycles_mode[c_col] > 0]
                if len(valid_base) == 0 or len(valid_curr) == 0: continue
                b = float(valid_base[b_col].mean())
                c = float(valid_curr[c_col].mean())
            else:
...
def chart_sequence_table(result):
    ...
    for seq_key,seq_name in seq_map.items():
        ...
        for label,col in normal_metrics.get(seq_key,[]):
            ...
            # Exclude 0-duration cycles from duration baselines
            if "duration" in col:
                valid_b = base_df[base_df[col] > 0]
                valid_c = new_df[new_df[col] > 0]
                if len(valid_b) == 0 or len(valid_c) == 0: continue
                b = float(valid_b[col].mean())
                c = float(valid_c[col].mean())
            else:
                b = float(base_df[col].mean())
                c = float(new_df[col].mean())
            if b==0 or np.isnan(b) or np.isnan(c): continue
            pct=((c-b)/abs(b))*100
