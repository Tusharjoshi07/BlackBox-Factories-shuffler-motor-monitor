...
def auto_analyse_latest(historical, baselines):
    ...
    results = {}
    for mode in sorted(latest_data["run_mode"].unique()):
...
def chart_multiday_trend(historical, baselines):
    ...
    for mode in [0.0,2.0,1.0]:
        ...
        fig.update_layout(
            title=f"Mode {int(mode)} — {MODE_NAMES.get(mode)} Return Torque Trend",
...
def chart_boxplots(historical, baselines):
    ...
    for mode in [0.0,2.0,1.0]:
        ...
        b_info=baselines.get(mode)
        alert=b_info["alert_threshold"] if b_info else None
        fig=go.Figure()
        for i,day in enumerate(mode_data["day"].unique()):
            day_data=mode_data[mode_data["day"]==day]["ret_torque_min"].dropna()
            fig.add_trace(go.Box(y=day_data,name=day,boxpoints="outliers",
                marker_color=px.colors.qualitative.Set2[i%8],line_width=2))
        if alert:
            fig.add_hline(y=alert,line_dash="dash",line_color="red",
                          annotation_text=f"Alert {round(alert,4)}")
        fig.update_layout(
            title=f"Mode {int(mode)} — {MODE_NAMES.get(mode)} Torque Distribution by Day",
            yaxis_title="Return Torque Min (Nm)",height=380,showlegend=False)
        figs[mode]=fig
    return figs
...
def chart_anomaly_scatter(cycles_df, mode, label):
    ...
    if len(anomalies)>0:
        fig.add_trace(go.Scatter(x=anomalies["start_time"],y=anomalies["total_duration"],
            ...
            showlegend=False),row=1,col=2)
    fig.update_layout(title=f"Mode {int(mode)} — Anomaly Detection ({n_anom} found) — {label}",height=380)
...
# DISPLAY MODE RESULT
# ─────────────────────────────────────────
def display_mode_result(result, show_root_cause=True):
...
with tab1:
    ...
    if auto_results:
        ...
        for col,(mode,result) in zip(banner_cols,auto_results.items()):
            ...
            with col:
                st.markdown(
                    f"**Mode {int(mode)} — {result['mode_name']}**\n\n"
                    f"{icon} **{status}**\n\n"
    ...
    for mode in [0.0,2.0,1.0]:
        ...
        for day in mode_data["day"].unique():
            ...
            summary_rows.append({
                "Mode":f"Mode {int(mode)} — {MODE_NAMES.get(mode)}",
    ...
    st.markdown("### 📦 Torque Distribution by Day")
...
with tab2:
    ...
    if df_raw is None:
        ...
        for col,(mode,mname) in zip([mc1,mc2,mc3],
            ...
            col.metric(f"Mode {int(mode)} — {mname}",f"{count:,} rows",
...
with tab3:
    ...
    if auto_results:
        for mode,result in auto_results.items():
            st.markdown(f"#### Mode {int(mode)} — {result['mode_name']}")
            display_mode_result(result, show_root_cause=True)
            st.markdown("---")
    else:
    ...
    if df_raw is not None and day_label:
        ...
        if st.button("🔍 Run Health Analysis on New Data",
            ...
            for mode in sorted(all_new["run_mode"].unique()):
                ...
                st.markdown(f"#### Mode {int(mode)} — {MODE_NAMES.get(mode,'Unknown')}")
                b_info=baselines.get(mode)
                if b_info is None:
                    ...
                    st.info(f"ℹ️ First time Mode {int(mode)} — using today as baseline.")
    ...
    if len(latest_data)>=10:
        for mode in sorted(latest_data["run_mode"].unique()):
            ...
            if out:
                ...
                st.plotly_chart(fig,use_container_width=True)
                if n_anom>0:
                    st.warning(f"⚠️ {n_anom} anomalous cycles in {latest_day} Mode {int(mode)}")
                else:
                    st.success(f"✅ No anomalies in {latest_day} Mode {int(mode)}")
    else:
    ...
    if df_raw is not None and day_label:
        ...
        if st.button("🚨 Detect Anomalies in New Data",use_container_width=True):
            ...
            if len(new_cycles)==0: st.error("❌ No cycles found.")
                ...
                for mode in sorted(new_cycles["run_mode"].unique()):
                    st.markdown(f"#### Mode {int(mode)} — {MODE_NAMES.get(mode,'Unknown')}")
        ...
        for mode in [0.0,2.0,1.0]:
            ...
            if anom_counts:
                ...
                fig=px.bar(anom_df,x="Day",y="Anomalies",
                           title=f"Mode {int(mode)} — Anomaly Count per Day",
                           color="Anomalies",color_continuous_scale="RdYlGn_r",text="Anomalies")
                fig.update_layout(height=300); st.plotly_chart(fig,use_container_width=True)
            out=chart_anomaly_scatter(mode_data,mode,"All Days")
            if out:
                fig,n_anom=out
                fig.update_layout(title=f"Mode {int(mode)} — All Historical ({n_anom} anomalies)")
                st.plotly_chart(fig,use_container_width=True)
            st.markdown("---")
